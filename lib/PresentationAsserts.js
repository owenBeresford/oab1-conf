...
// work out how to get this on first load
	pageRenderMetricsStart:function() {
//see with zero point values
		...
		try {
			let f1=function(list, obs ) {
				...
				for(let i in tmp) {
					console.log("name: "+ tmp[i].name +"; entryType: " + tmp[i].entryType +"; startTime: " + tmp[i].startTime +"; duration: "  + tmp[i].duration + "; start "+tmp[i].processingStart, tmp[i]);
			...
			console.warn("may not use pageRenderMetrics in this browser.");
			return null;
		}
	},
...
// @reference https://siongui.github.io/2017/05/02/javascript-search-visible-text-on-webpage/
...
// but that does assume people are writing good HTML
		...
		// recursive function adds data to buffer var
		}
...
console.log(sample, tt);
		return (tt ===null || tt===false|| (tt.length && tt.length===0));
// let re=/[0xD800-0xDBFF]|[0x110000-0x7FFFFFFF]/;
// @reference https://stackoverflow.com/questions/30738924/detecting-corrupt-characters-in-utf-8-encoded-text-file
// @reference https://unix.stackexchange.com/a/12545  << much better reference
...
// https://stackoverflow.com/questions/12470954/javascript-convert-ex-to-px
		...
		colourDifference2:function(t1, t2) {
			  /*
				 ...
				 @author me: I made the code abit more concise, as I don't think will read this.  Credit for knowledge goes to Jenn.   I  would like to extract the extra magic values
