 ...
 * @link http://owenberesford.me.uk/resource/ux-verify
...
const PresentationAsserts = {
    ...
    /**
     * create
     * Fake contor, as compat-JS doesn't have these

     * @param col ~ the TinyColor object
	 * @param pageLoad ~ bool, whether to setup page load metrics or not
     * @access public
     * @return 'this'
     */
    create: function (col, pageLoad) {
        this.colour = col;
  		if(pageLoad) {
			this.pageRenderMetricsStart();
		}
    	return this;
    },
     ...
     * @param sel ~ a querySelector string, mostly the same as CSS selectors
     * @access public
     * @return signed int OR "IMAGE" (where relevant)
     */
    itemColourContrast: function (sel) {
        ...
        return this.n.colourDifference2(t1, t2);
    },
...
// https://developer.mozilla.org/en-US/docs/Web/HTML/Element/label
    ...
    /**
     * gtEleByText
     * Extract an element by the text it holds, a popular feature of other framework
     *  
     * @access public
     * @param sample ~ The text sample 
	 * @param strict ~ How precise he matching is
     * @return null | the DOM element
     */	
	getEleByText:function(sample, strict) {
		let pth="";
		if(strict) { pth="//*[text()='"+sample+"']"; }
		else { pth="//*[contains(text(),'"+sample+"')]";  }
		
		try {
			var evaluator = new XPathEvaluator();
			var expression = evaluator.createExpression(pth);
			var result = expression.evaluate(document, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE);
			if(result.snapshotLength) {
// test return value here
				return result.snapshotItem(0);
			} else { return null; }

		} catch(e) {
			console.error("PresentationAsserts.getEleByText currently doesn't support MSIE", e);
			return null;
		}
	},

    /**
     * pageRenderMetricsStart
     * Setup the page metric system; call pageMetricsAcceptable to get any results.
 	 * Usually you want to initiate this via the con'tor.
     * @access public
     * @param verbose ~ log everything that happens
     * @return void
     */
	pageRenderMetricsStart:function(verbose) {
		this.metrics={ layout:[], first:[], largest:[] };
		const SELF=this;
		try {
			let f1=function(list, obs ) {
				let tmp=list.getEntries();
				for(let i in tmp) {
					verbose && console.log("name: "+ tmp[i].name +"; entryType: " + tmp[i].entryType +"; startTime: " + tmp[i].startTime +"; duration: "  + tmp[i].duration + "; start "+tmp[i].processingStart, tmp[i]);
					if( tmp[i].entryType==='layout-shift' && !tmp[i].hadRecentInput ) {
						SELF.metrics.layout.push({when:tmp[i].startTime, amount:tmp[i].value });
					}
					if( tmp[i].entryType==="largest-contentful-paint") {
						SELF.metrics.largest.push({when:tmp[i].startTime, amount:tmp[i].value });
					}
					if( tmp[i].entryType==="first-input") {
						SELF.metrics.first.push({when:tmp[i].startTime, amount:tmp[i].processedStart - tmp[i].startTime });
					}
				}

			};
			let p=new PerformanceObserver(f1 );
			p.observe({ entryTypes:['layout-shift', 'largest-contentful-paint', 'first-input'] });

		} catch(e) {
			console.warn("May not use any page render metrics in this browser.", e);
			return null;
		}
		return true;
	},
    ...
    /**
     * pageMetricsAcceptable
     * A function to convert captured page metric data into a boolean
 
     * @param type which metric, one of 'layout-shift', 'largest-contentful-paint', 'first-input'
     * @param threshold value to test against
     * @access public
     * @return boolean
     */
	pageMetricsAcceptable:function(type, threshold) {
		let count=0;
		switch(type) {
		case 'layout-shift':
			for(let i in this.metrics.layout) {
				count+=i.amount;
			}
		...
		case 'first-input':
			for(let i in this.metrics.first) {
				count+=i.amount;
			}
			return count < threshold;
			break;
		...
		default:
			console.error("ERROR unknown "+type+", nothing tested.");
		}
		return false;
	},
    ...
    /**
     * hasInvalidChars
     * Is the visible text on screen good UTF8?
 
     * @param sample OPTIONAL bit of test to measure rather than the whole doc
     * @link https://siongui.github.io/2017/05/02/javascript-search-visible-text-on-webpage/
     * @link https://stackoverflow.com/questions/30738924/detecting-corrupt-characters-in-utf-8-encoded-text-file
     * @link https://unix.stackexchange.com/a/12545  << much better reference
     * @access public
     * @return void
     */
	hasInvalidChars:function(sample) {
		var buffer="";
		...
		function isExcluded(elm) {
		  ...
		  if (elm.tagName == "NOSCRIPT") {
			// I am skipping this one, as JS is clearly loaded
			return true;
		  }
		...
		function traverse(elm) {
		  ...
		  if (elm.nodeType == Node.TEXT_NODE) {
			...
			buffer+=elm.nodeValue+" ";
		  }
		}

		if(sample) { buffer =sample; }
		else {
// maybe i should change the tag to <article>  
// but that does assume people are writing good HTML
			traverse(document.getElementsByTagName('body')[0]);
		// this recursive function adds data to buffer var
		}
		let re=/^(([\x00-\x7F])|([\xC2-\xDF][\x80-\xBF])|((([\xE0][\xA0-\xBF])|([\xED][\x80-\x9F])|([\xE1-\xEC\xEE-\xEF][\x80-\xBF]))([\x80-\xBF]))|((([\xF0][\x90-\xBF])|([\xF1-\xF3][\x80-\xBF])|([\xF4][\x80-\x8F]))([\x80-\xBF]{2})))*$/;
		let tt= buffer.match(re);
		return (tt ===null || tt===false|| (tt.length && tt.length===0));
	},
...
// https://github.com/feross/buffer/blob/master/index.js
    ...
    // see pageRenderMetricsStart pageMetricsAcceptable
    // very deprecated
    itemMovement: function (sel, samples) {
        throw new Error("No code found, pls feed true into con'tor; then call pageMetricsAcceptable()");
    },
...
// https://stackoverflow.com/questions/12470954/javascript-convert-ex-to-px
        ...
        colourDifference: function (t1, t2) {
            ...
            t2 = t2.substring(1).toLowerCase().match(/([a-f0-9]{2})/g);
            t1 = t1.substring(1).toLowerCase().match(/([a-f0-9]{2})/g);
            let t3 = [
         ...
         * @return int
         */
		colourDifference2:function(t1, t2) {
			  /*
				 @link https://codepen.io/jennz/pen/oNxVapJ
				 @author Jenn @geekgalgroks 
				 @author me: I made the code abit more concise, as I don't think anyone will read this.  Credit for knowledge goes to Jenn.   I  would like to extract the extra magic values
				 @date grabbed at 2020-10-12 (code may not live long)
			   */
			  ...
			  function gamma(sColor) {
				  if (sColor <= 0.03928 ) {
					  return sColor/12.92;
				  } else {
					  return (((sColor + 0.055)/1.055) ** 2.4);
				  }
			  }

			  function relativeLum(color) {
				  var sR = gamma( sRGB(color.substr(1,2)) );
				  var sG = gamma( sRGB(color.substr(3,2)) );
				  var sB = gamma( sRGB(color.substr(5,2)) );

				  return (0.2126 * sR) + (0.7152 * sG) + (0.0722 * sB);
			  }

			  var foreLum = relativeLum(t1);
			  var backLum = relativeLum(t2);
         ...
         * NB: from JS, I can't find out which font is actually in use
